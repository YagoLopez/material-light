...
<div><ml-radio #radio_scroll_head checked="true" (click)="setWaterfallEffectHeader()">
  Header with waterfall effect and white background</ml-radio></div>
<div><ml-radio #radio_fixed_head (click)="setFixedHeader()">
  Fixed Header (no waterfall) and white background</ml-radio></div>
<p><ml-radio (click)="setBackgroundTransparent()">Background transparent with image</ml-radio></p>

<p>There is also the option of having different nested drawers and headers in each page using auxiliary 
...
`//template
  ...
  mlHeaderTabContent: HTMLElement = this.mlLayout.querySelector('ml-header-tab-content') as HTMLElement;

  /**
   * Gives gray background color to <view-source/> buttons
   * @param color {string}
   */
  setBackgroundViewSourceBtns(color: string): void{
  ...
  setBackgroundTransparent() {
    ...
    this.mlHeaderTabContent.style.cssText = 'background-color: transparent; border: none; box-shadow: none';
    this.setBackgroundViewSourceBtns('darkgrey');
    this.disableRadioBtn(this.radioFixedHead);
  }
  setFixedHeader() {
    this.mlHeader.classList.remove('mdl-layout__header--waterfall');
  }
  setWaterfallEffectHeader() {
    ...
    this.mlHeaderTabContent.style.cssText = `@media (min-width: 700px) {background-color: white; margin: auto;
      width: 60%; padding: 25px 50px 50px; border: 1px solid lightgrey; box-shadow: 1px 1px 5px lightgrey; }`;
    this.setBackgroundViewSourceBtns('');
    this.enableRadioBtn(this.radioScrollHead);
  }
  ngOnDestroy(){
    // Restore initial header waterfall effect
    this.setWaterfallEffectHeader();
  }
}
